...
if src['type'] == 'dir':
    ...
    src_conf = '--src-host %s --src-port %s --src-profile-name %s'% (src['config']['host'], src['config']['port'], src['config']['profile_name'])

sink_conf = '--sink-host %s --sink-port %s --sink-profile-name %s' % (sink['config']['host'], sink['config']['port'], sink['config']['profile_name'])
