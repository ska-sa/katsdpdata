...
class FileWriterServer(DeviceServer):
    ...
    def request_capture_init(self, req):
        ...
        self._rx = spead2.recv.Stream(spead2.ThreadPool(4), bug_compat=spead2.BUG_COMPAT_PYSPEAD_0_5_2, max_heaps=2, ring_heaps=2)
        ...
        l0_heap = 16 * 17 * 2 * 32768 * 9
        memory_pool = spead2.MemoryPool(l0_heap, l0_heap+4096, 8, 8)
        self._rx.set_memory_pool(memory_pool)

        for endpoint in self._endpoints:
            self._rx.add_udp_reader(endpoint.port, bind_hostname=endpoint.host, buffer_size=157286400)
