...
def new_product_exists(product, solr_conn):
    ...
    results = solr_conn.search(
        ...
        f'CAS.ProductTypeName:{PRODUCT_NEW}')
    return results.hits
...
def create_l0_product_from_placeholder(rdb_product, placeholder_product, solr, s3_conn, update_set):
    product_id = f'{placeholder_product["Prefix"]}-sdp-l0-visibility'
...
def create_l1_product_from_placeholder(rdb_product, placeholder_product, solr, s3_conn, update_set):
...
def create_new_l1_products(product, solr, s3_conn, update_set):
    product_id = product['id'].replace(
        'sdp_l0', 'sdp_l1_flags').replace('sdp-l0', 'sdp-l1-flags')
    try:
        bucket = s3_conn.get_bucket(f'{product["id"]}')
    except:
        return
...
while res.nextCursorMark != cursorMark:
    ...
    for product in res:
        ...
        if new_product_exists(product, solr_conn):
            print(f'The product {product["CaptureBlockId"]} already exists. Doing nothing.')
            continue
        ...
        if placeholder_products.hits:
            ...
            create_l0_product_from_placeholder(product, p_product, solr_conn, s3_conn, update_set)
